...
Direct B4/SO(9) multiplicity extraction by sparse theta weights and the
triangular Weyl/ZQ recurrence.  This replaces the dense character matrix and
Gaussian solve used by partition_func_nansz_dynk_fix.py.

For a B4 highest weight lambda=[l,m,r,s], in doubled orthonormal coordinates

    lambda2 = (2l+2m+2r+s, 2m+2r+s, 2r+s, s),
    rho2    = (7,5,3,1),

the normalized multiplicity is

    a_lambda = [y^(lambda2+rho2)] A_rho(y) Z(y).

The supplied partition function is Z=Theta/ZQ.  ZQ has 153 distinct weights
and a unique lexicographic leading weight eta*=(4,0,0,0), so

    a_lambda = R[mu+eta*]
               - sum_{eta != eta*} q_eta b(mu+eta*-eta),
    R = A_rho Theta, mu=lambda2+rho2.

Every b(...) is a signed already-computed dominant multiplicity, zero on a
Weyl wall, or zero outside the ansatz.  Processing highest weights in
descending lexicographic order makes the recurrence triangular.

The expensive part is generating the required unnormalized theta weight
coefficients.  The theta numerator factorizes into four one-variable
bivariate series.  A runtime-compiled C kernel:
  ...
  * performs the 384-term Weyl sums and ordered 152-neighbor recurrence;
  * parallelizes pair and weight batches with pthreads.

The default implementation works modulo M=2^61-1, matching the original
solver. Exact mode reconstructs nonnegative integer coefficients by CRT and
certifies completion with the exact unrefined dimension sum. Candidate primes
can run concurrently. The program can automate a range of levels and export a
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sparse parallel B4 Weyl/ZQ multiplicity solver"
    )
    ...
    parser.add_argument(
        "--exact",
        action="store_true",
        help="automatically use enough primes and reconstruct exact integers by CRT",
    )
    parser.add_argument(
        "--verify-prime",
        action="store_true",
    ...
    for order in orders:
        ...
        if cached is not None:
            result = cached
            print(f"q={order}: loaded {len(result):,} coefficients from cache")
        else:
            started = time.perf_counter()
            if args.exact:
            ...
            if args.cache_dir:
                save_cached_level(args.cache_dir, order, result, args.exact)
        ...
        if expected_levels is not None and order in expected_levels:
            ...
            print(f"q={order}: validation mismatches={len(mismatches)}")
            if mismatches:
