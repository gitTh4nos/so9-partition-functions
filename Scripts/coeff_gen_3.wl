...
wolfram -script coeff_gen_3.wl -reps "{{p1,q1,r1,s1},{p2,q2,r2,s2},...}" -toplevel toplevel -kernels K
 -output outputfile -coeffFile ***.m
where the defaults are "{{p1,q1,r1,s1},{p2,q2,r2,s2},...}"= "{{0,0,0,0}}", toplevel=16, 
...
(*A logfile is written to coeff_gen_3.log *)
...
batchLog["========================================================"];
batchLog["coeff_gen_3.wl - Generate leading Rademacher"];
...
Expand/@(ComplexExpand[Re[#]]&/@coefs)]; shiftedSeriesQChar[dynkin_List,nList_List,q_Integer,NMax_Integer]:=Module[{base,dO,K,chi,coefs,j,m},base=shiftedSeriesQ[nList,q,NMax];(*previous routine*)dO=base["dOrder"];
...
coeff3array={{8,{0,0,0,0}},{4 4 2^3/2,{2,0,0,0}},{6 4^2 2^2/2,{1,1,0,0}},{4 4^3 2/2,{1,1,2,0}},{4^4/2,{1,1,1,1}}};
lca=Length[coeff3array];
rep3array=Flatten[Table[Join[{rep},coeff3],{rep,replist},{coeff3,coeff3array}],1];

LaunchKernels[Min[Length[rep3array],kernels]];
batchLog["Distributing definitions"];
DistributeDefinitions[expcoeffchar,toplevel,rep3array,lca,lr];

batchLog["Starting computation"];
coeff3listarray=ParallelTable[{rep3array[[i,1]],rep3array[[i,2]]expcoeffchar[rep3array[[i,1]],rep3array[[i,3]],3,toplevel]//Simplify},{i,rep3array//Length}];
coeff3list=Table[Join[{coeff3listarray[[lca i+1,1]]},Table[coeff3listarray[[lca i+j,2]],{j,lca}]],{i,0,lr-1}];
Export[output,Compress[coeff3list]];
