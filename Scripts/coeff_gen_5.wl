...
wolfram -script coeff_gen_5.wl -reps "{{p1,q1,r1,s1},{p2,q2,r2,s2},...}" -toplevel toplevel -kernels K
 -output outputfile -coeffFile ***.m
where the defaults are "{{p1,q1,r1,s1},{p2,q2,r2,s2},...}"= "{{0,0,0,0}}", toplevel=6, 
...
(*A logfile is written to coeff_gen_5.log *)


SetDirectory["../DataFiles/Super"];
(*Uncomment if you want to run for *)(*SetDirectory["~/YourWorkingDirectory/DataFiles/Super"];*)
(* ============ BATCH MODE SETUP ============ *)
...
batchLog["========================================================"];
batchLog["coeff_gen_5.wl - Generate leading Rademacher"];
...
(*Returns coefficient of t^(2k),k=0,...,NMax*)Table[Expand[f0 hlist[[k+1]]],{k,0,NMax}]];om={\[Omega][1],\[Omega][2],\[Omega][3],\[Omega][4]};
...
expcoeffchar[dynkin_List,ns_List,q_,level_]:=Module[{test},test=shiftedSeriesQChar[dynkin,ns,q,-24+level][[6]];Table[2^32/(384 (2\[Pi])^4) integrateCoefgen[ns,q,test[[i]],{\[Omega][1],\[Omega][2],\[Omega][3],\[Omega][4]}],{i,Length[test]}]]
(*Used to shift by a different multiple of ns,  5th Rademacher is first time we use this *)
expcoeffchara[dynkin_List,ns_List,a_,q_,level_]:=Module[{test},test=shiftedSeriesQChar[dynkin,ns,q,-24+level][[6]];Table[2^32/(384 (2\[Pi])^4) integrateCoefgen[-a ns,q,test[[i]],{\[Omega][1],\[Omega][2],\[Omega][3],\[Omega][4]}],{i,Length[test]}]]

(*-----------------------------------Main---------------------------------------------*)


(*List of shifts (second) and the degeneracy for each shift (first)*)(*List of shifts (second) and the degeneracy for each shift (first) and level of first appearence.*)
coeff5array={{8,{0,0,0,0},36},{4 4 2^3/2,{2,0,0,0},21},{4 4 2^3/2,{4,0,0,0},21},
{6 4^2 2^2/2,{1,1,0,0},12},{6 4^2 2^2/2,{2,2,0,0},12},{12 4 4 2^2/2,{4,2,0,0},10},
{4 4^3 2/2,{1,1,4,0},9},{12 4^2 4 2/2,{1,1,2,0},5},{12 4^2 4 2/2,{4,2,2,0},5},
{4 4^3 2/2,{2,2,2,0},9},{4^4/2,{1,1,1,1},12},{4 4^3 4/2,{1,1,4,2},6},{6 4^2 4^2/2,
{1,1,2,2},4},{4 4 4^3/2,{4,2,2,2},6},{4^4/2,{2,2,2,2},12}};
lca=Length[coeff5array];

rep5array=Flatten[Table[Join[{rep},coeff],{rep,replist},{coeff,coeff5array}],1];

qq=5;
coeff51listarray=ParallelTable[{rep5array[[i,1]],rep5array[[i,2]]expcoeffchara[rep5array[[i,1]],rep5array[[i,3]],-1,qq,toplevel-12+rep5array[[i,4]]]//Simplify},{i,rep5array//Length}];
coeff52listarray=ParallelTable[{rep5array[[i,1]],rep5array[[i,2]]expcoeffchara[rep5array[[i,1]],rep5array[[i,3]],2,qq,toplevel-12+rep5array[[i,4]]]//Simplify},{i,rep5array//Length}];

coeff51list=Table[Join[{coeff51listarray[[lca i+1,1]]},Table[coeff51listarray[[lca i+j,2]],{j,lca}]],{i,0,lr-1}];
coeff52list=Table[Join[{coeff52listarray[[lca i+1,1]]},Table[coeff52listarray[[lca i+j,2]],{j,lca}]],{i,0,lr-1}];

Export[output,Compress[{coeff51list,coeff52list}]];
